            ...
            <div class="p-4 sm:p-8 sm:rounded-2xl 0 shadow-2xl shadow-gray-800" style="background-color: #f56e0f;">
                <div class="w-full space-y-8">
                    ...
                    <div class="w-full flex justify-end">
                            <a href="{{ route('kamar.create') }}" class="bg-white mx-3 p-1 px-4 rounded-xl"><h1 class="text-2xl">+</h1></a>
                    </div>
                    <form class="w-full flex gap-4" role="search" method="GET" action="{{ route('kamar') }}">
                        @csrf
                        <input class="flex-[4] rounded-md px-4 py-1 border-black/10 shadow shadow-xl" style="background-color: #f56e0f;" type="search" placeholder="Search" aria-label="Search" id="keywordkamar" name="keywordkamar">
                                    ...
                                    @foreach ($kamar as $kamar)
                                        <tr class="border-b border-red-800">
                                            ...
                                            <td class="flex px-4 py-3 space-x-2">
                                                ...
                                                <div id="default-modal-{{ $kamar->id }}" tabindex="-1" aria-hidden="true" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
                                                    <div class="relative p-4 w-full max-w-2xl max-h-full">
                                                        ...
                                                        <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                                                            ...
                                                            <!-- Modal footer -->
                                                            <form action="{{ route('kamar.destroy', $kamar->id) }}" method="POST" class="p-6">
                                                                @csrf
                                                                @method('delete')
                                                ...
                                                <x-primary-link href="{{ route('kamar.edit', $kamar->id) }}">{{ __('Edit') }}</x-primary-link>
                                            </td>
                                        </tr>
                                    @endforeach                            
                                </tbody>
